...
def polar2cart2D(polarImage2D, cc, cart_shape):
    cc = np.round(cc).astype(int)
    polarImage2D = polarImage2D.flatten()
    image2D = np.zeros(cart_shape)
    mask2D = np.zeros(cart_shape)
    image2D[cc[0], cc[1]] = polarImage2D
    mask2D[cc[0], cc[1]] = 1
    image2D = image2D.reshape(cart_shape)
    mask2D = mask2D.reshape(cart_shape)
    return image2D, mask2D


def mask_refiner(prob, mask=None, iterations=1, neigh=(5, 2, 2), neigh_sigmas=(5, 0.5, 0.5), weight=50, comp_std=0):
    ...
    mask is in the same shape with prob. Each time, the the mask location will not be updated
    ...
    for i in range(iterations):
        ...
        new_prob = new_prob
        new_prob = new_prob - np.max(new_prob, axis=0, keepdims=True)
        new_prob = np.exp(new_prob)
        new_prob = new_prob / np.sum(new_prob, axis=0, keepdims=True)

        if mask is not None:
            prob = mask * orig_prob + (1 - mask) * new_prob
        else:
            prob = new_prob
