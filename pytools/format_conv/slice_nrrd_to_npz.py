...
def crop_along_cline(filename):
    ...
    # for img_idx in range(20, c - 21):  # skip 20 layers both ends
    for img_idx in range(4 * num_skip, c - 4 * num_skip - 1):
        ...
        if label_count < srcmsk_count_z_median / 2:  # do not slice bad label
            continue
        # slice_name = basename + '_slice_' + "{0:03d}".format(img_idx) + '.npz'
        ...
        # npz_slice = srcdata[:, :, img_idx - 20:img_idx + 21:5]  # TODO: modify second 20 to 21
        npz_slice = srcdata[:, :,
